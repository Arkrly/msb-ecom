...
The keys live in `.claude/settings.json` (project) or `/home/arkrly/Projects/msb-ecom/.opencode/settings.json` (global) — **not** in `.planning/config.json`:
...
See @/home/arkrly/Projects/msb-ecom/.opencode/gsd-core/references/planner-guidance.md for Granularity Calibration table (Coarse/Standard/Fine plans-per-phase).
